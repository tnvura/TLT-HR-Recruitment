...
export function InterviewerInfo({ candidateId, candidateStatus }: InterviewerInfoProps) {
  ...
  // Statuses where interviewer should not be editable (offer phase and beyond)
  const nonEditableStatuses = ["to_offer", "pending_approval", "offer_sent", "offer_accepted", "offer_rejected", "hired"];
  const isStatusLocked = candidateStatus && nonEditableStatuses.includes(candidateStatus);
  ...
  const canEdit = canUpdate && !isStatusLocked;
      ...
      <CardContent>
        {isEditing ? (
          ...
          <div className="space-y-3">
            ...
            {isStatusLocked && (
              <div className="pt-2 border-t text-xs text-amber-700 bg-amber-50 p-2 rounded">
                <p className="font-medium">Interviewer cannot be changed</p>
                <p className="text-amber-600">Candidate has progressed to offer stage</p>
              </div>
            )}
          </div>
        )}
