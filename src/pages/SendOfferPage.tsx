...
import { SelectHRManagerDialog } from "@/components/SelectHRManagerDialog";
...
export default function SendOfferPage() {
  ...
  const [showHRManagerDialog, setShowHRManagerDialog] = useState(false);
    ...
    try {
      ...
      if (feedbackData) {
        setFormData((prev) => ({
          ...
          employment_type: feedbackData.employment_type || "",
          temp_start_date: feedbackData.temp_start_date || "",
          temp_end_date: feedbackData.temp_end_date || "",
          position_type: feedbackData.position_type || "",
        }));
      }
    } catch (error: any) {
      toast({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate email fields (optional fields, but if provided must be valid)
    ...
    try {
      ...
      // Add temp dates to validation if employment_type is temporary
      ...
      // Show HR Manager selection dialog instead of submitting immediately
      setPendingSubmission(true);
      setShowHRManagerDialog(true);
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message,
  ...
  const handleHRManagerSelected = async (hrManagerId: string, hrManagerEmail: string) => {
    setShowHRManagerDialog(false);
    ...
    try {
      ...
      const { data: proposalData, error: proposalError } = await (supabase as any)
        ...
        .insert({
          ...
          assigned_hr_manager_id: hrManagerId,
          assigned_hr_manager_email: hrManagerEmail,
          created_by: user.id,
      ...
      await (supabase as any)
        ...
        .insert({
          ...
          notes: `Job offer submitted for approval to ${hrManagerEmail}`,
        });

      // Create in-app notification for assigned HR Manager
      ...
      console.log("🔔 Attempting to send offer_submitted_for_approval notification...");
      await emailNotifications.notifyOfferSubmittedForApproval(
        candidateId!,
        hrManagerEmail,
        hrManagerEmail, // Using email as name since we don't fetch the name
        user.email!,
        candidate,
        {
          position_offered: formData.position_offered,
      ...
      console.log("🔔 Offer notification call completed");
  ...
  return (
    <div className="container mx-auto px-6 py-8 max-w-6xl">
      ...
      <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setDirectReportEmailError(""); // Clear error on change
                    }}
                    onBlur={(e) => {
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setManagerEmailError(""); // Clear error on change
                    }}
                    onBlur={(e) => {
                      if (e.target.value && !isValidEmail(e.target.value)) {
                        setManagerEmailError("Please enter a valid email address");
        ...
        {/* HR Manager Selection Dialog */}
        <SelectHRManagerDialog
          open={showHRManagerDialog}
          onOpenChange={(open) => {
            setShowHRManagerDialog(open);
            if (!open) {
              setPendingSubmission(false);
            }
          }}
          onConfirm={handleHRManagerSelected}
        />
      </div>
    </div>
  );
